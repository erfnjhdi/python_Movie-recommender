...
def load_data(path="data/movies.csv", credits_path="data/credits.csv"):
    ...
    try:
        ...
        if "cast" in credits.columns:
            credits["cast_names"] = credits["cast"].apply(
                lambda x: extract_cast_names(x, top_n=3)
            )
        if "crew" in credits.columns:
            credits["director"] = credits["crew"].apply(extract_director)

        credits = credits[["title", "cast_names", "director"]].drop_duplicates(
...
def get_recommendations(
    ...
    recs["_combined_score"] = (recs["_similarity_score"] * (1 - title_weight)) + (
        recs["_title_similarity"] * title_weight
    )
